...
""")
    ...
    def _odometry_callback(self, odometry):
        ...
        if self._initial_pose_received:
            ...
            if t_odom + t_filter > 0.1:
                print("Filter cycle overran timeslot")
                print(f"Odometry update: {t_odom:.6f} seconds")
                print(f"Particle update: {t_filter:.6f} seconds")
            # Calculate and print mean error and spread
            # mean_error = self.calculate_mean_error(self._particle_filter.particlecloud, self._particle_filter.estimatedpose.pose.pose)
            # spread = self.calculate_spread(self._particle_filter.particlecloud)
    ...
    # def calculate_mean_error(self, particles, estimated_position):
    #     errors = [
    #         np.sqrt((particle.position.x - estimated_position.position.x) ** 2 +
    #                 (particle.position.y - estimated_position.position.y) ** 2)
    #         for particle in particles.poses
    #     ]
    #     return np.mean(errors)

    # def calculate_spread(self, particles):
    #     x_positions = [particle.position.x for particle in particles.poses]
    #     y_positions = [particle.position.y for particle in particles.poses]
